...
from .custom_analyzer import K8sCustomAnalyzer  # Ensure this file exists
...
from .custom_analyzer import K8sCustomAnalyzer
import logging
...
def register_natural_language_tools(mcp, kubeconfig_path):
    """
    Registers the MCP tools for the KubeTalk agent.
    """

    @mcp.tool()
    async def analyze_kubernetes_cluster(namespace: str = "all", filter_resource: str = None, anonymize: bool = False):
        """
        Runs a comprehensive diagnostic scan (K8sGPT-style) of the cluster or specific namespace.
        Use this to troubleshoot Pods, Services, Deployments, and Nodes without fetching large logs.
        """
        logger.info(f"Starting native analysis for namespace: {namespace}")
        try:
            analyzer = K8sCustomAnalyzer(kubeconfig_path)
    ...
    @mcp.tool()
    async def get_pod_logs(pod_name: str, namespace: str, tail_lines: int = 100):
        """
        Fetches the last few lines of pod logs. 
        Use this instead of full logs to avoid the 2.4M token overflow.
        """
        try:
            # Use the verified path for the logger
    ...
    # --- Your Existing Tool Logic Below ---
    # Note: I have kept your existing process_query logic for compatibility
